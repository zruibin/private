...
<!--BEGIN_DATA
{
    "create_date": "2016-07-15 17:04", 
    "modify_date": "2016-07-15 17:04", 
    "is_top": "0", 
    "summary": "用树型模型管理App数字和红点提示", 
    "tags": "Android、数据结构", 
    "file_name": "用树型模型管理App数字和红点提示.md"
}
END_DATA-->

####<p>原文出处：<a href='http://zhangtielei.com/posts/blog-badge-number-tree.html' target='blank'>用树型模型管理App数字和红点提示</a></p>
...
点击[下载链接](http://pan.baidu.com/s/1hspYLve)。
...
![Badge Count Demo](./image/badge_count_screenshot.png)
...
![Badge Dot Demo](./image/badge_dot_screenshot.png)
...
上面说的问题，我们在[微爱](http://welove520.com)App开发的初期也遇到过。后来，我们重新审视了App中红点和数字展示的结构，使用树型结构来看待它，让维护工作变得简单。

一个App的页面本身就是分级的，对于页面的访问路径本质上就是个树型结构。

![Badge Number Tree结构图](./image/badge_number_tree.png)
...
这个树型的模型可以如下表述：
  ...
  * Badge Number根据所属业务类型的不同，分属不同的大类（Category）。每个大类内的Badge 
 
Number类型type分配在同一个类型区间内。比如上面树型结构图中2，3，4节点就分别对应三个业务类型，也就是三个大类，它们对应的类型区间分别为[A, C], [X, Y], [R, T]。再举一个实际的例子，比如微信朋友圈是一个业务大类，里面的Badge Number类型包括：有人评论我（数字），有人给我点赞（数字），好友有新消息发布（红点），等。
...
**原创文章，转载请注明出处！**   
**本文链接：**<http://zhangtielei.com/posts/blog-badge-number-tree.html>   
