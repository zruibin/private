...
<!--BEGIN_DATA
{
    "create_date": "2020-12-07 13:54", 
    "modify_date": "2020-12-07 13:54", 
    "is_top": "0", 
    "summary": "C++的对象布局", 
    "tags": "C/C++", 
    "file_name": "C++的对象布局.md"
}
END_DATA-->

#### <p>原文出处：<a href='https://zhuanlan.zhihu.com/p/156880783' target='blank'>C++的对象布局</a></p>
...
![](./image/20201207-135420-8.jpg)

**offset_to_top(0)**：表示当前这个虚函数表（BaseA，Derive）地址距离对象顶部地址的偏移量，因为对象的头部就是虚函数表的指针，所以偏移量为0。

再注意这里的**RTTI**中有了两项，表示BaseA和Derive的虚表地址是相同的，BaseA类里的虚函数和Derive类里的虚函数都在这个链条下，截至到offset_to_top(-16)之前都是BaseA和Derive的虚函数表。
