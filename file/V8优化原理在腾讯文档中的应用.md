...
<!--BEGIN_DATA
{
    "create_date": "2024-08-01 22:12", 
    "modify_date": "2024-08-01 22:12", 
    "is_top": "0", 
    "summary": "V8优化原理在腾讯文档中的应用.md", 
    "tags": "C/C++、Web", 
    "file_name": "V8优化原理在腾讯文档中的应用"
}
END_DATA-->

#### <p>原文出处：<a href='https://km.woa.com/articles/show/608458' target='blank'>怎么让你的 js 跑得快十倍 —— V8 优化原理在腾讯文档中的应用</a></p>

| 导语 前段时间依据 V8 的 JIT 策略来优化重构文档数据层点读相关实现，将 OOXML 属性点读性能优化到原来的十倍，使 doc 整体排版耗时获得
10~15% 的下降；本文将从 V8 整体架构出发，深入浅出 V8 对象模型，从汇编细节点出其 ICs 优化细节以及原理，最后根据这些优化原理来编写超快的 JS 代码 🚀

写在前面：前段时间依据 JS 引擎的 JIT 策略来优化重构文档数据层点读相关实现，将 OOXML 属性点读性能优化到原来的十倍，使 doc
整体排版耗时获得 10~15% 的下降。 ![](./image/20240801-2212-2.png)

本文将从 V8 整体架构出发，深入浅出 V8 对象模型，从汇编细节点出其 ICs 优化细节以及原理，最后根据这些优化原理来编写超快的 JS 代码 🚀

## 一、V8 compiler pipeline

js 代码从源码到执行 —— v8 编译器管线：

![](./image/20240801-2212-4.png)
...
![](./image/20240801-2212-6.png)

#### c) V8 Ignition 的理解

Ignition 会将代码理解编译为 bytecode :
...
总之为了充分表达 js 动态特性以及方便优化为 CPU 能直接裸跑的汇编，v8 引入了 bytecode 这个层次，它比 AST 更接近物理机，因为它没有层次嵌套，是一种基于栈的带累加寄存器的指令集。 (ignition is a bytecode stack machine with an accumulator register)
...
![](./image/20240801-2212-9.png)

对应的是 V8 源码里的这个枚举:

![](./image/20240801-2212-10.png)

![](./image/20240801-2212-11.png)

从开发视角来看，一个函数最佳的 status 应该是 `00000000000001010001 (81)` 即

![](./image/20240801-2212-12.png)

### 4. %HasFastProperties(obj);

%HasFastProperties 可以用来打印对象是否是 Fast Properties 模式

![](./image/20240801-2212-13.png)
...
![](./image/20240801-2212-14.png)

备注：

  1. `void *` 是 C 的 `any`，强转比较多，请忽略 warning

  2. 从这也可以看到超过 `2^31` 的整数或者浮点数就不能用 SMI 了，此时会装箱为特殊的 HeapObject 放进堆里 (x64 机器上是 2^32 有些微妙不同, 参考 [v8.dev/blog/pointer-...](https://v8.dev/blog/pointer-compression))

  3. 你可以通过 `%DebugPrint({})` 和 `%DebugPrint(123)` 来看看其指针数值是不是整数，然后你会发现所有对象的指针数值都是奇数 Tagged Pointer ( 实际上 heap 内的都是奇数 heapdump 里也能看到这个细节 )

## 五、V8 基于 assumption 的 JIT 机器码优化

我们先来看这个例子，一个 add(x,y) 函数，如果运行期间出现了多种类型的传参，那么会导致代码变慢：

![](./image/20240801-2212-15.png)

我们可以看到，L15 速度慢了非常多，比一开始的 66ms 慢了几倍。

原因：

  1. 一开始只会传数字的时候，**V8 会假设这是数字加法，可以极致优化。（66 毫秒可以跑完）**

  2. L13 传入其他参数，上述假设会被推翻，此时打印一次优化状态可以看看出现了 `反优化`，在 L13 执行的时候实际走的是 Ignition 解释器去跑的。

  3. 执行 L15 for 循环走了足够多次后，V8 收集到足够的 `feedback` 后会重新建立假设来做优化，不过这次的假设是「入参可能是 number 也可能是 string」—— 这意味着调用的时候要多判断入参类型是 string 还是 number 从而导致了最终的性能劣化 （一模一样的代码要 243 毫秒才跑完，慢了有三倍吧）
...
![](./image/20240801-2212-17.png)

一个更直观的伪码是:

```
// 运行了足够多次收集到反馈信息:「a 大概率是整数」
fn test_jit(a: any): i32 {
  if (a 不是整数) goto 兜底回滚到 ignition 解释器模式运行;
  // 执行汇编级别原生整数加法，速度超快
  return X86_ASM_ADD(a, 0x1234);
}
```
...
此外，从汇编和伪码来看也能知道，如果 assumption 经常被打破，性能会变差，而且如果入参的类型太多也会导致 checkpoint / type-guard 的逻辑变复杂，因此也能解释前面 `add(a, b)` 为什么打破一次之后性能就不如一开始的性能了。

### 2. 哪里可以打印所谓 feedback ？

通过 `%DebugPrint` 可以看到

![](./image/20240801-2212-18.png)

当打破这个 assumption 后，会变成 Any

![](./image/20240801-2212-19.png)

### 3. 多态 return 会导致优化效果打折吗？

不会

![](./image/20240801-2212-20.png)

### 4. feedback slot 里的 {Mono|Poly|Mega|}morphic 是？

  1. Monomorphic 单态：指参数的类型只有一种，不会变

  2. Polymorphic 多态：指参数的类型有多种 (比较短的 union type)

  3. Megamorphic 巨态：指参数的类型非常复杂 (非常长的 union type) 
...
![](./image/20240801-2212-21.png)

### 6. 反优化太多次怎么办？

根据这篇文章 [V8 function optimization - Blog by Kemal Erdem](https://erdem.pl/2019/08/v-8-function-optimization) 如果某个函数「反优化」超过 5 次后，v8 以后就不再会对这个函数做优化了，不过我无法复现他说的这个情况，可能是老版本的 v8 的表现，node16 不会这样，不管怎样只要 run 了足够多次都会 turbofanned，只是如果「曾经传的参数类型太 union typed」会导致优化效果出现非常大的折损

### 7. 什么时候会启动 TutboFan ？

前面我们已经知道了「运行足够多次」会触发优化，而这只是其中一种情况，具体可以参考 v8 里 ShouldOptimize 的实现，里面有详细定义何时启动优化：

![](./image/20240801-2212-22.png)

作为开发视角来看：

  1. L371 已经优化过的代码不会再优化

  2. L375 这段逻辑决定是否启用 maglev (具体见备注)

  3. L386 通过参数主动禁用/或者省电模式等这类不会优化 ( 比如 node --v8-options="--turbo_filter=xxxxx" )

  4. L394 运行足够多次才会优化 (还有个配置项 efficiency_mode_delay_turbofan 配置延迟多久启动 TurboFan）

  5. L402 太长的函数不会优化

备注：maglev 是去年 chrome v8 团队搞的新特性 —— 编译层次优化，总的来说就是根据 feedback 对机器码的编译层次做精细控制来达到更好的优化效果，下图是 v8 团队发布的 benchmark 对比：

![](./image/20240801-2212-23.png)

具体可参考 [v8.dev/blog/maglev](https://v8.dev/blog/maglev)

### 8. 编译后的代码会占内存吗？

会的，而且有时候这部分内存占用非常多，这也是 Chrome 经常被调侃为内存杀手的重要原因之一，以 `qq.com` 为例，具体对应是 `heapdump` 里的 `(compiled code)` 包含了编译后的代码内存占用：

![](./image/20240801-2212-24.png)
...
![](./image/20240801-2212-25.png)

C 会将 struct 理解为一段连续的线性 buffer 结构，并在上面根据字段的类型来划分好从下标的哪里到哪里是哪个字段（对齐），因此在编译point.x 的时候会改成 base+4 的方式进行属性访问，如下图所示，时间复杂度是 O(1) 的：

![](./image/20240801-2212-26.png)
...
好了就上面这样简单弄一下就搞出了好多问题了。

从下面开始正式进入，V8 是如何描述对象，参见下文

### 2. JSObject 与 named-properties & indexed-elements

在 js 标准里 Array 是一类特殊的 Object，但出于性能考虑 V8 底层针对对象和数组的处理是不同的：

  1. 所谓 indexed-elements 指的是数组元素（以数字下标作为 key）存储于 `*elements`，是一段线性内存空间，可以直接用下标直接访问，查找速度非常快

  2. 而其他的普通成员所谓 named-properties 则存储于 `*properties` 查找速度比较慢，需要遍历对比

如下图所示，JSObject：

![](./image/20240801-2212-29.png)

在 V8 里：

  1. Array-indexed 的属性存储在 `*elements` 里，查找速度快；Named Properties 则存储在 `*properties` 里，查找速度慢

  2. Properties/Elements 这两个结构可以是数组，但有时候也会变成字典（比如稀疏数组场景，线性内存空间就不够性能了）

  3. 每个 JSObject 都有一个 *hiddenClass，用于保存对象的 Shapes
...
![](./image/20240801-2212-33.png)

当出现不同走向的时候，此时会单独开一个 branch 来描述这种情况，此时 o1 和 o2 就不再一样了：

![](./image/20240801-2212-34.png)

### 6. V8 对象模型总结

从前文的讨论，可以得到的结论：

  1. V8 使用 JSObject 来描述对象，上面有若干个字段（除了上面那些还有 prototype 原型链那些，相对不那么重要，就没画出）

  2. V8 还使用 Tagged Pointer 来描述对象指针（前文有提）

  3. named properties 成员存储在 *properties 里，可以为数组，也可以为字典

  4. named properties 也可以存储在 `in-object properties` 里，可以动态增长。  
  
  5. 数字下标成员存储在 `*elements` 里，可以为数组，也可以为字典（稀疏数组场景）
...
你可能还会问：在调用优化后的 distance2 的时候具体要怎么确定传入的 p1 p2 的 shapes 是否有变化？还记得前面那个 _0xABCD_ 吗？没错，编译后的汇编 checkpoint 就是直接判断传入对象的 hidden classs 指针数值是不是 _0xABCD_，如果不是就触发「反优化」兜底解释器模式运行即可。
...
静态和动态两种写法风格可能会有几倍甚至上百倍的差距，如果业务里有大几百万次的调用test，优化后能省不少毫秒，比如下面这个「简化的服务发现」例子有近百倍的差距：

![](./image/20240801-2212-36.png)

原因是 s2.js 里那些属性访问都被 ICs 技术优化成 `O(1)` 访问了，速度很快 —— 为了探究内部的 ICs 相关汇编逻辑，尝试输出serviecMap 的 Hidden Class (V8 里 hidden class 别名是 Map) 以及汇编源码：

![](./image/20240801-2212-37.png)
...
![](./image/20240801-2212-39.png) 如果知道 ICs 技术内涵的话，理解 Fast Properties 和 SlowProperties (或者称字典模式) 就不会有困难了。

右图描述了 JSObject 的主要构造：当把对象成员存储到 `in-object properties` 的时候，此时称对象是 `FastProperties` 模式，这意味着对象访问 V8 会在合适的时候将其 Inline Cache 到优化后的汇编里； 反之，当成员存储到 `*properties` 的时候，此时称为 `Slow Properties`，此时就不会对这类对象做 inline cache 优化了，此时对象访问性能最差（因为要遍历 `*properties` 字典，通常慢几十到几百倍，取决于对象成员数量）

我们可以用 `%HasFastProperties` 来打印对象是否是 Fast Properties 模式，如下图所示

![](./image/20240801-2212-40.png)

delete 会将对象转为 slow properties 模式，为什么呢？因为 delete 带来的问题可太多了，缓存技术最怕的就是delete，如图所示：

![](./image/20240801-2212-41.png)
...
![](./image/20240801-2212-46.png)

在这些优化技术的加持上，safari jscore 某些情况下甚至会比 chrome v8 还要快:

![](./image/20240801-2212-47.png)
...
热点函数会优先走 TurboFan 编译为机器码，性能会更好，要如何利用好这个特性？ 
将项目里的一些高频原子操作拆成独立函数，人为制造热点代码，比如计算点距离，单位换算等等这些需要高性能的地方：

![](./image/20240801-2212-48.png)

### 2. 函数拆解

除了前面提到的热区之外，拆解后的函数如果足够短，那么 V8 在调用的时候会做 inline 展开优化，节省一次调用栈开销。

![](./image/20240801-2212-49.png)

### 3. 减少函数状态（Mono）

从前面的 add 的例子我们可以知道，V8 TurboFan 优化是基于 assumption 的，应该尽量保持函数的单态性(monophonic)，或者说减少函数的状态，具体来说高频函数不要传 Union Types 作为参数。（这个不够准确，最好是不要打破参数的 V8 内部类型表示以及汇编 checkpoint，比如一会传浮点数、一会传 SMI 这样即使都是 number 也会打破 v8 的假设，因为 v8 内部实现的浮点数会装箱，而小整数 SMI 不会，两者的汇编逻辑不一样）

推荐使用 TypeScript 来写 js 应用，限制函数的入参类型可以有效保证函数的单态性质，更容易编写高性能的 js 代码

### 4. 保持对象赋值顺序不变（Hidden Class）

赋值顺序的不同会产生不同的 Hidden Class 链，不同的链不能做 ICs 优化

### 5. class 里的字段声明最好加上一个默认值
...
### 10. 尽量保证对象就只作用在一个函数内（逃逸分析）

![](./image/20240801-2212-50.png)
...
通过重构点读逻辑，简化过程并尽量命中引擎的优化逻辑，从底层优化点读性能。

在 Chrome 下:

![](./image/20240801-2212-51.png)

而 Safari 用的 WebKit JSCore 现在也有基于 LLVM 后端的 JIT 和 ICs 技术，因此这项优化在 Safari 上同样有效果：

![](./image/20240801-2212-52.png)

汇总指标:

![](./image/20240801-2212-53.png)

最终业务效果： 到具体排版火焰图里是 get 过程达到 50%+ 的效果，get 从原来的置顶项优化到中间去了，综合来看点读占的越多最后优化效果就更好

![](./image/20240801-2212-54.png)

现网排版耗时下降 10~15% :

![](./image/20240801-2212-55.png)

## 十二、资料参考, EOF

这块参考了大量资料，有的地方只有源码里才有，这里简单列一下:

  1. [Maps (Hidden Classes) in V8 · V8](https://v8.dev/docs/hidden-classes)

  2. [Fast properties in V8 · V8](https://v8.dev/blog/fast-properties)

  3. [Google Docs - TurboFan TechTalk presentation](https://docs.google.com/presentation/d/1sOEF4MlF7LeO7uq-uThJSulJlTh--wgLeaVibsbb3tc/edit#slide=id.g5499b9c42_01170)

  4. [GitHub - v8blink/v8-JavaScript-Documents: 《Chrome V8源码》系列技术文章](https://github.com/v8blink/v8-JavaScript-Documents)

  5. [Pointer Compression in V8 · V8](https://v8.dev/blog/pointer-compression)

  6. [浏览器工作原理与实践——V8工作原理 - 女王控的博客](https://blog.towavephone.com/browser-working-principle-v8/)

  7. [Google开源的JavaScript引擎——V8 - 知乎](https://www.zhihu.com/zvideo/1408790742785916928)

  8. [Escape Analysis in V8](https://www.jfokus.se/jfokus18/preso/Escape-Analysis-in-V8.pdf)

  9. [Speculation in JavaScriptCore | WebKit](https://webkit.org/blog/10308/speculation-in-javascriptcore/)

  10. [Slack tracking in V8 · V8](https://v8.dev/blog/slack-tracking)

  11. [Ignition Design Doc - Google Docs](https://docs.google.com/document/d/11T2CRex9hXxoJwbYqVQ32yIPMh0uouUZLdyrtmMoL44/edit#heading=h.6jz9dj3bnr8t)

附件(1)

[v8.drawio](./zip/v8.drawio)
