...
<!--BEGIN_DATA
{
    "create_date": "2022-03-17 22:20", 
    "modify_date": "2022-03-17 22:20", 
    "is_top": "0", 
    "summary": "mediasoup源码分析-初始化、建立连接及媒体数据的处理流程", 
    "tags": "音视频", 
    "file_name": "mediasoup源码分析-初始化、建立连接及媒体数据的处理流程.md"
}
END_DATA-->
...
![](./image/20220317-2220-13.png)  
![](./image/20220317-2220-14.png)
...
IceServer主要用于ICE，建立与客户端的连接。在一般的WebRTC中，端与端的建立是通过`Full ICE`建立连接的。而mediasoup使用的是`lite ICE`，所以在ICE中，mediasoup角色为`controlled`，而客户端角色为`controlling`。
...
![](./image/20220317-2220-24.png)

### WebRtcTransport发送数据过程

![](./image/20220317-2220-25.png)
...
收到的DTLS数据包，可能是握手数据包，也可能是Sctp数据包。如果是后者，将数据写入BIO后，让dtls解密接收到的数据，然后通过`SSL_read()`获取解密后的数据，最后通过OnDtlsTransportApplicationDataReceived函数将解密后的数据送至WebRtcTransport。
...
先将数据通过`SSL_write`加密后写入到BIO中，经过DTLS加密，再把加密后的数据通过libuv发送出去。
