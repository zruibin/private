...
<!--BEGIN_DATA
{
    "create_date": "2020-03-21 14:19", 
    "modify_date": "2020-03-21 14:19", 
    "is_top": "0", 
    "summary": "调度系统设计精要", 
    "tags": "系统、算法", 
    "file_name": "调度系统设计精要.md"
}
END_DATA-->

####<p>原文出处：<a href='https://draveness.me/system-design-scheduler' target='blank'>调度系统设计精要.md</a></p>
...
![scheduler-works-and-resources](./image/20200321-141952-2.png)
...
![linux-banner](./image/20200321-141952-3.jpg)
...
![kubernetes-banner](./image/20200321-141952-4.png)
...
![cooperative-and-preemptive](./image/20200321-141952-5.png)
...
![cooperative-scheduling](./image/20200321-141953-6.png)
...
![preemptive-scheduling](./image/20200321-141953-7.png)
...
![single-schedule](./image/20200321-141953-8.png)
...
![multi-scheduler](./image/20200321-141953-9.png)
...
![multi-scheduler-with-coarse-grained](./image/20200321-141953-10.png)
...
![work-stealing-scheduler](./image/20200321-141954-11.png)
...
![how-scheduler-works](./image/20200321-141954-12.png)
...
决策模块会根据状态模块收集的上下文和任务的规格做出调度决策，需要注意的是做出的**调度决策只是在当下有效**，在未来某个时间点，状态的改变可能会导致之前做的决策不符合任务的需求，例如：当我们使用 Kubernetes调度器将工作负载调度到某些节点上，这些节点可能由于网络问题突然不可用，该节点上的工作负载也就不能正常工作，即调度决策失效。
...
![scheduling-framework](./image/20200321-141954-13.png)
...
![scheduler-and-descheduler](./image/20200321-141954-14.png)

**图 15 - 调度器与反调度器**

反调度器的引入使得整个调度系统变得更加健壮。调度器负责根据当前的状态做出正确的调度决策，反调度器根据当前的状态移除错误的调度决策，它们的作用看起来相反，但是目的都是为任务调度更合适的资源。

反调度器的使用没有那么广泛，实际的应用场景也比较有限。作者第一次发现这个概念是在 Kubernetes 孵化的[descheduler](https://github.com/kubernetes-sigs/descheduler)项目4中，不过因为反调度器移除调度关系可能会影响正在运行的线上服务，所以 Kubernetes 也只会在特定场景下使用。
...
![mid-term-scheduler](./image/20200321-141954-15.png)
...
无论是进程还是线程，在 Linux 中都被看做是 `[task_struct`](https://github.com/draveness/linux-archive/blob/master/0.01/include/linux/sched.h#L77) 结构体，所有的调度进程都存储在上限仅为[64](https://github.com/draveness/linux-archive/blob/master/0.01/include/linux/sched.h#L4) 的数组中，调度器能够处理的进程上限也只有 64 个。


![linux-initial-scheduler](./image/20200321-141954-16.png)

**图 17 - 最初的进程调度器**

上述函数会先唤醒获得信号的可中断进程，然后从队列倒序查找计数器 `counter` 最大的可执行进程，`counter`是进程能够占用的时间切片数量，该函数会根据时间切片的值执行不同的逻辑：

  * 如果最大的 `counter` 时间切片大于 0，调用汇编语言的实现的 `[switch_to`](https://github.com/draveness/linux-archive/blob/master/0.01/include/linux/sched.h#L168) 切换进程；
  * 如果最大的 `counter` 时间切片等于 0，意味着所有进程的可执行时间都为 0，那么所有进程都会获得新的时间切片；

Linux 操作系统的计时器会每隔 10ms 触发一次 `[do_timer`](https://github.com/draveness/linux-archive/blob/master/0.01/kernel/sched.c#L159) 将当前正在运行进程的 `counter`减一，当前进程的计数器归零时就会重新触发调度。
...
$O(1)$ 调度器在 v2.6.0 到 v2.6.22 的 Linux 内核中使用了四年的时间，它能够在常数时间内完成进程调度，你可以在`[sched.h`](https://github.com/draveness/linux-archive/blob/master/2.6.8.1/include/linux/sched.h) 和`[sched.c`](https://github.com/draveness/linux-
...
![runqueue-and-prio-array](./image/20200321-141954-17.png)
...
![global-runqueue-and-local-runqueue](./image/20200321-141954-18.png)
...
完全公平调度器（Completely Fair Scheduler，CFS）是 v2.6.23版本被合入内核的调度器，也是内核的默认进程调度器，它的目的是最大化 CPU 利用率和交互的性能10。Linux 内核版本 v2.6.23 中的 CFS由以下的多个文件组成：
  ...
  * [include/linux/sched.h](https://github.com/draveness/linux-archive/blob/master/2.6.23/include/linux/sched.h)
  * [kernel/sched_stats.h](https://github.com/draveness/linux-archive/blob/master/2.6.23/kernel/sched_stats.h)
  * [kernel/sched.c](https://github.com/draveness/linux-archive/blob/master/2.6.23/kernel/sched.c)
  * [kernel/sched_fair.c](https://github.com/draveness/linux-archive/blob/master/2.6.23/kernel/sched_fair.c)
  * [kernel/sched_idletask.c](https://github.com/draveness/linux-archive/blob/master/2.6.23/kernel/sched_idletask.c)
  * [kernel/sched_rt.c](https://github.com/draveness/linux-archive/blob/master/2.6.23/kernel/sched_rt.c)
...
调度类中包含任务的初始化、入队和出队等函数，这里的设计与面向对象中的设计稍微有些相似。内核中包含`SCHED_NORMAL`、`SCHED_BATCH`、`SCHED_IDLE`、`SCHED_FIFO` 和 `SCHED_RR`调度类，这些不同的调度类分别实现了 `[sched_class`](https://github.com/draveness/linux-archive/blob/master/2.6.23/include/linux/sched.h#L855) 中的函数以提供不同的调度行为。

######小结

本节介绍了操作系统调度器的设计原理以及演进的历史，从 2007 年合入 CFS 到现在已经过去了很长时间，[目前的调度器](https://github.com/torvalds/linux/tree/master/kernel/sched)12也变得更加复杂，社区也在不断改进进程调度器。

我们可以从 Linux调度器的演进的过程看到主流系统架构的变化，最初几十行代码的调度器就能完成基本的调度功能，而现在要使用几万行代码来完成复杂的调度，保证系统的低延时和高吞吐量。

由于篇幅有限，我们很难对操作系统的调度器进行面面俱到的分析，你可以在 [这里](https://github.com/draveness/linux-archive) 找到作者使用的 Linux 源代码，亲自动手分析不同版本的进程调度器。

######延伸阅读

  * [What is long term scheduler, short term scheduler and mid term term scheduler in OS?](https://www.quora.com/What-is-long-term-scheduler-short-term-scheduler-and-mid-term-term-scheduler-in-OS)
  ...
  * [Understanding the Linux 2.6.8.1 CPU Scheduler](https://github.com/bdaehlie/linux-cpu-scheduler-docs/blob/master/linux_cpu_scheduler.pdf)
  ...
  * [The Linux desktop may soon be a lot faster](https://web.archive.org/web/20110223082612/http://blogs.computerworld.com/17371/the_linux_desktop_may_soon_be_a_lot_faster)
  * [Modular Scheduler Core and Completely Fair Scheduler](https://lwn.net/Articles/230501/)
...
![go-and-erlang](./image/20200321-141955-19.png)
...
多线程调度器的主要问题是调度时的锁竞争会严重浪费资源，[Scalable Go Scheduler Design Doc](http://golang.org/s/go11sched) 中对调度器做的性能测试发现 14% 的时间都花费在 `[runtime.futex`](https://github.com/golang/go/blob/2fffba7fe19690e038314d17a117d6b87979c89f/src/pkg/runtime/os_linux.h#L9) 上15，该调度器有以下问题需要解决：
...
![golang-gmp](./image/20200321-141955-20.png)
...
这种实现方式虽然增加了运行时的复杂度，但是实现相对简单，也没有带来过多的额外开销，总体来看还是比较成功的实现，也在 Go 语言中使用了 10几个版本。因为这里的抢占是通过编译器插入函数实现的，还是需要函数调用作为入口才能触发抢占，所以这是一种**协作式的抢占式调度**。
...
上述 9 个步骤展示了基于信号的抢占式调度的执行过程。除了分析抢占的过程之外，我们还需要讨论一下抢占信号的选择，提案根据以下的四个原因选择
`SIGURG` 作为触发异步抢占的信号19；
...
![go-numa-scheduler-architecture](./image/20200321-141955-21.png)
...
> 本节内容选择[《Go 语言设计与实现》](https://draveness.me/golang/)一书中的 [Go语言调度器实现原理](https://draveness.me/golang/docs/part3-runtime/ch06-concurrency/golang-goroutine/)，你可以点击链接了解更多与 Go 语言设计与实现原理相关的内容。
...
![container-orchestration](./image/20200321-141955-22.png)
...
Kubernetes 调度器的演变过程比较简单，我们可以将它的演进过程分成以下的两个阶段：
  ...
  * 基于谓词和优先级的调度器（Predicates and Priorities） · [v1.0.0](https://github.com/kubernetes/kubernetes/blob/v1.0.0/plugin/pkg/scheduler) ~ [v1.14.0](https://github.com/kubernetes/kubernetes/tree/v1.14.0/pkg/scheduler)
  * 基于调度框架的调度器（Scheduling Framework） · [v1.15.0](https://github.com/kubernetes/kubernetes/tree/v1.15.0/pkg/scheduler) ~ [至今](https://github.com/kubernetes/kubernetes/blob/master/pkg/scheduler)


Kubernetes 从 v1.0.0 版本发布到 v1.14.0，总共 15 个版本一直都在使用谓词和优先级来管理不同的调度算法，知道 v1.15.0开始引入调度框架（Alpha 功能）来重构现有的调度器。我们在这里将以[v1.14.0](https://github.com/kubernetes/kubernetes/tree/v1.14.0/pkg/scheduler)版本的谓词和优先级和[v1.17.0](https://github.com/kubernetes/kubernetes/tree/v1.17.0/pkg/scheduler)版本的调度框架分析调度器的演进过程。

#######谓词和优先级算法

谓词（Predicates）和优先级（Priorities）调度器是从 Kubernetes v1.0.0 发布时就存在的模式，[v1.14.0](https://github.com/kubernetes/kubernetes/tree/v1.14.0/pkg/scheduler)的最后实现与最开始的设计也没有太多区别。然而从 v1.0.0 到 v1.14.0 期间也引入了很多改进：

  * 调度器扩展（Scheduler Extender） · v1.2.0 - [Scheduler extension](https://github.com/kubernetes/kubernetes/commit/cadc24e9fd7f2bccc972df4d67985aa33a4cd823)
    * 通过调用外部调度器扩展的方式改变调度器的决策；
  * Map-Reduce 优先级算法 · v1.5.0 - [MapReduce-like scheduler priority functions](https://github.com/kubernetes/kubernetes/commit/33c710adf06ec844ed65127aaab14fc2bc2bf79c)
    * 为调度器的优先级算法支持 Map-Reduce 的计算方式，通过引入可并行的 Map 阶段优化调度器的计算性能；
  * 调度器迁移 · v1.10.0 - [Move scheduler code out of plugin directory](https://github.com/kubernetes/kubernetes/commit/30b89d830b7fd0827576853d6a0db44b66a90d3d)
...
![predicates-and-priorities-scheduling](./image/20200321-141955-23.png)
...
![kubernetes-scheduling-queue](./image/20200321-141955-24.png)
...
我们可以将调度器中的 `[Scheduler.scheduleOne`](https://github.com/kubernetes/kubernetes/blob/b7a1d61462be9006eed62be3a08f5bb222798624/pkg/scheduler/scheduler.go#L561) 方法作为入口分析基于调度框架的调度器实现，每次调用该方法都会完成一遍为 Pod 调度节点的全部流程，我们将该函数的执行过程分成调度和绑定两个阶段，首先是调度器的调度阶段：
  ...
  2. 调用 `[genericScheduler.Schedule`](https://github.com/kubernetes/kubernetes/blob/b7a1d61462be9006eed62be3a08f5bb222798624/pkg/scheduler/core/generic_scheduler.go#L165) 函数选择节点，该过程会执行 `PreFilter`、`Filter`、`PostFilter`、`Score` 四个扩展点的插件；
  3. 调用 `[framework.RunReservePlugins`](https://github.com/kubernetes/kubernetes/blob/b7a1d61462be9006eed62be3a08f5bb222798624/pkg/scheduler/framework/v1alpha1/framework.go#L690) 函数运行 `Reserve` 插件用于保留资源并进入绑定阶段（绑定阶段运行时间较长，避免资源被抢占）； 
    * 如果运行失败执行，调用 `[framework.RunUnreservePlugins`](https://github.com/kubernetes/kubernetes/blob/b7a1d61462be9006eed62be3a08f5bb222798624/pkg/scheduler/framework/v1alpha1/framework.go#L717) 函数运行 `Unreserve` 插件；
...
因为每一次调度决策都会改变上下文，所以该阶段 Kubernetes 需要串行执行。而绑定阶段就是实现调度的过程了，我们会创建一个新的 Goroutine并行执行绑定循环：
  ...
  3. 调用 `[Scheduler.bind`](https://github.com/kubernetes/kubernetes/blob/b7a1d61462be9006eed62be3a08f5bb222798624/pkg/scheduler/scheduler.go#L509) 函数将 Pod 绑定到 Node 上完成调度，绑定的过程会执行 `PreBind`、`Bind` 和 `PostBind` 三个扩展点的插件；
...
######延伸阅读
  ...
  * [Scheduling Framework #624](https://github.com/kubernetes/enhancements/issues/624)
  ...
  * [Scheduler extender Document](https://github.com/kubernetes/kubernetes/blob/cadc24e9fd7f2bccc972df4d67985aa33a4cd823/docs/design/scheduler_extender.md)
...
![schedulers](./image/20200321-141955-25.png)
...
#####延伸阅读
  ...
  * [Randomized Work Stealing versus Sharing in Large-scale Systems with Non-exponential Job Sizes](https://arxiv.org/abs/1810.13186)
  * [Scalable work stealing](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.151.2178&rep=rep1&type=pdf)


  1. [Wikipedia: Scheduling (computing)](https://en.wikipedia.org/wiki/Scheduling_\(computing\)) ↩
  2. [Scheduling: Theory, Algorithms, and Systems](https://book.douban.com/subject/3058593/) ↩
  3. [Scheduling multithreaded computations by work stealing](http://supertech.csail.mit.edu/papers/steal.pdf) ↩
  4. [descheduler · GitHub](https://github.com/kubernetes-sigs/descheduler) ↩
  5. [How Linux handles threads and process scheduling](https://stackoverflow.com/questions/8463741/how-linux-handles-threads-and-process-scheduling) ↩
  6. [schedule · Linux 0.01](https://github.com/draveness/linux-archive/blob/master/0.01/kernel/sched.c#L68) ↩
  7. [O(n) 调度器遍历就绪队列](https://github.com/draveness/linux-archive/blob/master/2.4.0/kernel/sched.c#L569) ↩
  8. [Understanding the Linux Kernel, Third Edition.](https://www.amazon.com/Understanding-Linux-Kernel-Third-Daniel/dp/0596005652) ↩
  9. [Introducing the 2.6 Kernel](https://www.linuxjournal.com/article/6530) ↩
  10. [Wikipedia: Completely Fair Scheduler](https://en.wikipedia.org/wiki/Completely_Fair_Scheduler) ↩
  11. [Wikipedia: Red-black tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) ↩
  12. [Linux Scheduler](https://github.com/torvalds/linux/tree/master/kernel/sched) ↩
  13. [Communicating sequential processes](https://dl.acm.org/doi/10.1145/359576.359585) ↩
  14. [为什么使用通信来共享内存 · Why’s THE Design?](https://draveness.me/whys-the-design-communication-shared-memory) ↩
  15. [Scalable Go Scheduler Design Doc](http://golang.org/s/go11sched) ↩
  16. [Pre-emption in the scheduler](https://golang.org/doc/go1.2#preemption) ↩
  17. [Go Preemptive Scheduler Design Doc](https://docs.google.com/document/d/1ETuA2IOmnaQ4j81AtTGT40Y4_Jr6_IDASEKg0t0dBR8/edit#heading=h.3pilqarbrc9h) ↩
  18. [runtime: goroutines do not get scheduled for a long time for no obvious reason](https://github.com/golang/go/issues/4711#issuecomment-66073943) ↩
  19. [Proposal: Non-cooperative goroutine preemption](https://github.com/golang/proposal/blob/master/design/24543-non-cooperative-preemption.md#other-considerations) ↩
  20. [Proposal: Conservative inner-frame scanning for non-cooperative goroutine preemption](https://github.com/golang/proposal/blob/master/design/24543/conservative-inner-frame.md) ↩
  21. [NUMA-aware scheduler for Go](https://docs.google.com/document/u/0/d/1d3iI2QWURgDIsSR6G2275vMeQ_X7w-qxM2Vp7iGwwuM/pub) ↩
  22. [Borg, Omega, and Kubernetes](https://queue.acm.org/detail.cfm?id=2898444) ↩
  23. [谈 Kubernetes 的架构设计与实现原理](https://draveness.me/understanding-kubernetes) ↩
  24. [Scheduling Framework](https://github.com/kubernetes/enhancements/blob/master/keps/sig-scheduling/20180409-scheduling-framework.md) ↩
