...
<!--BEGIN_DATA
{
    "create_date": "2022-04-07 22:20", 
    "modify_date": "2022-04-07 22:20", 
    "is_top": "0", 
    "summary": "UE4网络模块", 
    "tags": "C/C++", 
    "file_name": "UE4网络模块.md"
}
END_DATA-->

#### <p>原文出处：<a href='https://km.woa.com/articles/show/518440' target='blank'>UE4网络模块</a></p>
...
![](./image/20220407-2220-0.png)

由上图可以看出，确实是 TCP 可能产生合并或拆分，送达的 UDP 数据报带有正确的边界，测试代码我放到了 Github上：<https://github.com/zhyingkun/lua-5.3.5/blob/master/demo/uvwrap/stick.lua>
...
### Actor 承载的同步内容
  ...
  1. Actor 自己（ActorChannel 同步的第⼀个 Bunch 附带）
  2. Properties、DeltaProperty、RPC
  3. SubObjects（必须是 UObject）
    1. SubObject 自己（SubObject 同步的第一个 ContentBlock 附带）
    2. Properties、DeltaProperty、RPC
    3. Sub SubObjects（由此可构成一棵 UObject 树）
...
其中：
  ...
  3. 循环从 Packet 中读出数据
    1. 根据 Bunch 结构 从 Packet 读出状态位（包括 ChIndex）以及此 RawBunch 的数据大小
    2. 从 Packet 读出 RawBunch 的数据，此时得到一个 RawBunch
    3. 根据 ChIndex 去找对应 Channel，找不到就用 Bunch 结构 的信息创建一个（仅限 ActorChannel）
    4. 将 RawBunch 丢给该 Channel 去处理（以下针对 ActorChannel）
      ...
      4. 循环从 Bunch 中读出 ContentBlock
        ...
        4. 反序列化过程依赖反射信息
        5. 属性同步则先将旧的属性复制出来，执行反序列化去更新属性，再比较是否有变化，有变化则将该 Property 加入 RepNotifies 数组
        6. 是 RPC 则走 Replicator::ReceivedRPC，最终通过 Object->ProcessEvent(Function, Parms);执行 RPC
...
网络同步 SapwnActor 不立刻走 BeginPlay 图：（图）  

![](./image/20220407-2220-11.png)
...
针对 UProperty 配置，只配置 Replicated 没效果，只实现 GetLifetimeReplicatedProps 会编译报错  
另外，被 Actor 管理同步的 UObject 也可以有自己的 RPC。

## Replicate 一个 Actor

ReplicateActor 图：（图）  
![](./image/20220407-2220-12.png)
...
其中：
  ...
  3. 序列化该 Actor 需要同步的属性
    1. 用当前属性值与 staticbuff 记录的缓存值进行比对，得到发生变化的属性列表
    2. 讲属性变化列表记录到属性变化记录管理器中，并 Merge 属性历史中需要 Resend 的属性
    3. 序列化这些属性
  4. 序列化 UnreliableMulticastRPC
  5. 调用 Actor 的 ReplicateSubobjects 接口
    1. 针对每一个 UObject 进行序列化
    2. 首先找到 UObject 对应的 FObjectReplicator
    3. 序列化 UObject 需要同步的属性（这一步与序列化 Actor 需要同步的属性类似，也有属性变化历史记录）
    4. 序列化 UnreliableMulticastRPC
    5. 讲 ContentBlock 写入 Bunch 中，这样多个 SubObject 会有多个 ContentBlock
  6. 记录需要删除的 SubObject 到 Bunch
  7. SendBunch
...
还有，Actor 已同步到客户端的情况下，同时设置属性和调用非 UM 的 RPC 以及 UMRPC，一般顺序为：  

非 UnreliableMulticast 的 `RPC => OnRep_XXX => UnreliableMulticast` 的 RPC（此执行顺序受网络环境影响，写代码不可依赖此顺序）
