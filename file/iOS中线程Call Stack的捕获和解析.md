...
<!--BEGIN_DATA
{
    "create_date": "2016-08-09 15:38", 
    "modify_date": "2016-08-09 15:38", 
    "is_top": "0", 
    "summary": "iOS中线程Call Stack的捕获和解析", 
    "tags": "iOS", 
    "file_name": "iOS中线程Call Stack的捕获和解析.md"
}
END_DATA-->

####<p>原文出处：<a href='http://blog.csdn.net/jasonblog/article/details/49909163' target='blank'>iOS中线程Call Stack的捕获和解析(一)</a></p>
...
我的理解是应该包含当前线程的执行地址，并且从这个地址可以一级一级回溯到线程的入口地址，这样就反向构成了一条链：线程入口执行某个方法，然后逐级嵌套调用到当前现
场。

![Call_stack_layout_svg](./image/6fe88303be1468f692bf80099fdf0d18a5e6efad.png)
（图片来源于维基百科）
...
_The function thread_get_state returns the execution state (e.g. the machine
registers) of target_thread as specified by flavor._
...
_The frame pointer register (x29) must always address a valid frame record,
although some functions–such as leaf functions or tail calls–may elect not to
create an entry in this list. As a result, stack traces will always be
meaningful, even without debug information_

而在ARMv7/v6上描述到：

_The function calling conventions used in the ARMv6 environment are the same
as those used in the Procedure Call Standard for the ARM Architecture (release
1.07), with the following exceptions:_
...
####<p>原文出处：<a href='http://blog.csdn.net/jasonblog/article/details/49909209' target='blank'>iOS中线程Call Stack的捕获和解析（二）</a></p>
...
做这一块时也是查阅了很多链接和书籍，包括但不限于：
  ...
  * [《OS X ABI Mach-O File Format Reference》](https://developer.apple.com/library/mac/documentation/DeveloperTools/Conceptual/MachORuntime/)
  ...
  * [《程序员的自我修养》](http://book.douban.com/subject/3652388/)——这本几年前读过的，又一次从书架上拿下来温习，主要是用来对比确认；

  * [《The Mac Hacker’s Handbook》](http://www.amazon.cn/The-Mac-Hacker-s-Handbook-Miller-Charles/dp/0470395362/ref=sr_1_1?ie=UTF8&qid=1447826833&sr=8-1&keywords=mac+hacker+handbook)

  * [《Mac OS X and iOS Internals》](http://www.amazon.cn/Mac-OS-X-and-iOS-Internals-To-the-Apple-s-Core-Levin-Jonathan/dp/1118057651/ref=pd_sim_sbs_14_1?ie=UTF8&dpID=51y%2BHC3RdML&dpSrc=sims&preST=_AC_UL160_SR126%2C160_&refRID=0RVDY2AYPWF1ZEY43J5C)
...
![mach_o_segments](./image/48e6115f37b8833334c75453d1b1872ad7f495c6.gif)（图片来源于
苹果官方文档）
...
_A segment defines a range of bytes in a Mach-O file and the addresses and
memory protection attributes at which those bytes are mapped into virtual
memory when the dynamic linker loads the application. As such, segments are
always virtual memory page aligned. A segment contains zero or more sections._
...
`Segments and sections are normally accessed by name. Segments, by convention,
are named using all uppercase letters preceded by two underscores (for
example, _TEXT); sections should be named using all lowercase letters preceded
by two underscores (for example, _text). This naming convention is standard,
although not required for the tools to operate correctly.`
...
也就是说，我们需要结合__LINKEDIT segment_command（见上面结构描述）和LC_SYMTAB
load_command（见下面结构描述）来定位符号表：
...
如上述引用描述，LC_SYMTAB和_LINKEDIT中的偏移量都是文件偏移量，所以要获得内存中符号表和字符串表的地址，我们先将LC_SYMTAB的symo
ff和stroff分别减去LINKEDIT的fileoff得到虚拟地址偏移量，然后再加上_LINKEDIT的vmoffset得到虚拟地址。当然，要得到最终的
实际内存地址，还需要加上基于ASLR的偏移量。
